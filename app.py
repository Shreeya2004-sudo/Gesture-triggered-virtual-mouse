...
class Controller:
    ...
    def get_position(hand_result):
        ...
        if distsq <= 25:
            ...
            ratio = 0.03 * (distsq ** 0.5)
        else:
            ratio = 1.2
        # Step 1: calculate raw movement
        x = x_old + delta_x * ratio
        y = y_old + delta_y * ratio

        # Step 2: apply smoothing (damping)
        x = int(0.7 * x + 0.3 * x_old)
        y = int(0.7 * y + 0.3 * y_old)
       
        margin = 0.08  # 8% edge zone
        hx = hand_result.landmark[9].x
        hy = hand_result.landmark[9].y
        boost = 35  # pixels
        if hx < margin:
            x -= boost
        elif hx > 1 - margin:
    ...
    # ---------------- VOICE TOGGLE (ERGONOMIC) ----------------
        if gesture == Gest.PINCH_MINOR and not Controller.voice_lock:
            Controller.voice_mode = not Controller.voice_mode
            Controller.voice_lock = True

            if Controller.voice_mode:
...
def voice_typing():
    ...
    recognizer.pause_threshold = 0.6   # shorter silence
    recognizer.energy_threshold = 300

    while Controller.voice_mode:
        try:
            with mic as source:
                recognizer.adjust_for_ambient_noise(source, duration=0.2)
                audio = recognizer.listen(
                    source,
                    timeout=3,              # don’t wait forever
                    phrase_time_limit=5     # max seconds per sentence
                )
